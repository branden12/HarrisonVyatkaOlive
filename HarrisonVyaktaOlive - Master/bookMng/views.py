...
def addfavorites(request, book_id):
    ...
    duplicates = Favorite.objects.filter(book_id=book.id)
        ...
        if duplicates:
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    favorite.username = request.user
                    favorite.book_id = book.id

                except Exception:
                    pass
                favorite.save()
                return HttpResponseRedirect('/addfavoritespost?submitted=True')
    else:
        form = FavoriteForm()
        if 'submitted' in request.GET:
            submitted = True
...
def remove_favorites(request, book_id):
    ...
    book = Book.objects.get(id=book_id)
    favorite = Favorite.objects.get(title=book.name)
    ...
    return render(request,
                  'bookMng/feature_favorites/removefavorites.html',
                 {
                    'item_list': MainMenu.objects.all(),
                    'books': books,
                 })